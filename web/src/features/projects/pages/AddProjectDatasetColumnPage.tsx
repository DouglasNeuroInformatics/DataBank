...
//     contentTrimKeys.forEach((trimKey) => {
//       resContent[trimKey] = {
//         deps: ['selected'],
//         kind: 'dynamic',
//         render: (data: { selected: { has: (arg0: string) => any } }) => {
...
//             return null;
//           }
//           if (data.selected.has(trimKey.slice(0, -4))) {
//             return {
//               kind: 'boolean',
//               label: trimKey,
//               variant: 'radio'
...
/**
 *
 * need to get the columns page info by sending a get request to the backend
 * the backend should return an array of JSON
 * [
 *      {
 *          columnId: string,
 *          columnType: ENUM,
 ...
 * ]
 *
 * the table should allow search and sort on column Name
 * it would be best to allow for a list of column names
 * each row represent a column and we should be able to select any rows
 * there should be an option to select all
 * there should be a button to move on when finish selecting, the data should be
 * passed to the next page to customize options for each column, so we need the
 * data to contain column names and column ids
 *
 * It would be better if we can group columns of the table, for example, string
 * column summary can be in a group and select to expand
 *
 * the column table header looks like this
 *    Column Name | Count | Null_count | IntSummary | etc
 * []      X
 * []      Y
 *
 * is it possible for the user to save the current selection progress so they
 * can come back to the current state?
 */
...
const AddProjectDatasetColumnPage = () => {
  ...
  useEffect(() => {
    axios
      .get<ProjectColumn[]>(`/v1/datasets/columns/${params.datasetId}`)
      .then((response) => {
  ...
  return (
    <div className="container mx-auto py-10">
      <ProjectColumnsTable columns={projectColumnDefs} data={data} />
    </div>
  );
};
